...
public class EnemyEffectController : MonoBehaviour
{
    ...
    protected void RangeCheck()
    {
        ...
        Collider[] cols = Physics.OverlapSphere(transform.position, radius, 1 << LayerMask.NameToLayer(LayerType.hero.ToString()));
        for (int i = 0; i < cols.Length; i++)
        {
            ...
            if ((cross.y >= 0 && deg <= angle || cross.y <= 0 && deg <= angle) && dis <= radius)
            {
                ...
                cols[i].GetComponent<Hero>().Hurt(v3.normalized * data.repel, data.damage,true);
            }
        }
    }
}
